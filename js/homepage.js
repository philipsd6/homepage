...
function updateBackgroundWithGeoPattern() {
	...
	var colorChoices = [
		tinycolor(pattern.color).brighten(40),
...
function createLinks(target, links) {
	...
	for (var link of links) {
		...
		const atext = document.createTextNode("\u00A0" + link.name);
		a.appendChild(icon);
		a.appendChild(atext);
		a.href = link.url;
...
function createServices(target, services) {
	const grid = document.createElement("div");
	grid.className = "grid";
